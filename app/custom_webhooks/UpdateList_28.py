...
async def update_list_28(fast_bitrix, params: Request.query_params, request_bitrix=None):
    ...
    if not sum_element_info:
        sum_element_id = await fast_bitrix.call('lists.element.add', {
            ...
            'FIELDS': {
                ...
                fields_info['Факт. план ремонта ТС, шт']['FIELD_ID']: 0,
                ...
                fields_info['Новое ОБС с учетом КЗ, План']['FIELD_ID']: 0,
                fields_info['Новое ОБС с учетом КЗ, Факт']['FIELD_ID']: 0,
    ...
    await fast_bitrix.call('lists.element.update', {
        ...
        'FIELDS': {
            ...
            fields_info['Факт. план ремонта ТС, шт']['FIELD_ID']: sum(map(lambda element: int(get_element_value('Факт. план ремонта ТС, шт', fields_info, element)), date_range_elements_info)),
            ...
            fields_info['Новое ОБС с учетом КЗ, План']['FIELD_ID']: sum(map(lambda element: int(get_element_value('Новое ОБС с учетом КЗ, План', fields_info, element)), date_range_elements_info)),
            fields_info['Новое ОБС с учетом КЗ, Факт']['FIELD_ID']: sum(map(lambda element: int(get_element_value('Новое ОБС с учетом КЗ, Факт', fields_info, element)), date_range_elements_info)),
